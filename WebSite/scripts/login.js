...
function authenticate() {
    ...
    var resource2 = localAuthContext.getResourceForEndpoint("mpdigitaltwins101.azurewebsites.net");
    localAuthContext.acquireToken(resource2, function(error, token) {
        if(error || !token){
            handleTokenError(error);
        }
        else {
            console.log("Succesfully retrieved token2")}
        }
    );
    if(user){
